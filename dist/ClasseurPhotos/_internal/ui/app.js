...
$("#addPerson").onclick = async () => {
  const name = (prompt("Prénom de la personne (servira de nom de dossier) :") || "").trim();
  if (!name) return;
  const paths = await api().pick_images();
  if (!paths || !paths.length) return;
  state.curName = name; state.selections = {};
  $("#modalName").textContent = name;
  $("#faces").innerHTML = `<div class="hint">Détection des visages…</div>`;
  $("#modal").hidden = false;
...
$("#modalSave").onclick = async () => {
  ...
  if (!sels.length) { toast("Sélectionne au moins un visage."); return; }
  const r = await api().enroll(state.curName, sels);
  if (r.error) { toast(r.error); return; }
  closeModal(); toast(`${state.curName} enregistré(e).`); refreshPeople();
};
